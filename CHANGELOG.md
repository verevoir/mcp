...
- **The design gate fires against any corpus, and runs alongside the antagonistic review** (STDIO-510). Two changes that make the STDIO-507 gate genuinely useful:
  - **Sources-backed runner.** The gate tooling was loaded by dynamic import from a _local checkout_ only, so a remote (github / notion) corpus silently fell back to the practices review (observed live: the design-pack gate never fired because the configured corpus is remote). It's now read through the **same `@verevoir/sources` adapter the corpus itself uses** (`pickSourceAdapter` / `resolveSourceEnv` via the router): the whole zero-dependency `tooling/design/*.mjs` set is fetched, materialised to a temp dir, and dynamically imported (cross-file imports resolve as siblings). So the gate fires whether the corpus is local OR remote. `fetchDesignToolingFiles` + the pure `pickToolingFiles` are testable seams; `loadDesignTooling` takes an injectable fetch.
