...
- **Plan-first coordinator** (STDIO-520, ADR 016): plan a DAG of capability nodes, gate it before any spend, execute the gated enact per node in parallel (`plan-executor`, `plan-coordinator`, `coord:plan` bin). The coordinator plans and holds no production, so it cannot inline past a capability's gate — the structural fix for the improvising coordinator's inline-bypass (opus produced a token set itself, skipping the gate, for $19.92; mistral/sonnet delegated to the gated enact and converged). Planning runs on the reasoning tier regardless of coordinator (plan-smart, execute-cheap); independent nodes run concurrently (wall-clock = critical-path depth).
- **Verify-loop convergence** (STDIO-521): the re-produce directive threads the worker's own previous output and demands the corrected artifact only — a smaller worker (DeepSeek) was derailing into _discussing_ the findings instead of re-emitting the work; this is the convergence unlock. A non-converged verify now returns `ok: false` (no longer ships gate-failing output stamped as success). Plus 429/5xx exponential backoff on the openai-compat worker tier, worker action-token counting, and `extractTokenJson` finding the DTCG set among a multi-part run's output (measures the produced artifact, not the coordinator's summary).

## 0.73.0 — 2026-07-01 (partial — superseded by 0.74.0, do not use)
...
- **Delegation-discovery steer: route at produce-time, and coupling is a decompose signal not a keep-it signal** (STDIO-516). The handoff guidance was missed in practice: a capable session, told to delegate 'mechanical or bulk' work, self-classified ~1300 lines of coupled templating as _careful, not bulk_, so the trigger excluded exactly the work that should have been handed down — and the escape clause ('keep inline the judgement-heavy pieces') sanctioned keeping it. Rewritten as a **precondition tied to the volume of what you are about to generate** (not whether it 'feels mechanical'), and it names the exact trap: _"too coupled / too careful to delegate" is the signal to DECOMPOSE, not to self-generate_ — force a shared scaffold stage that **documents the common contract**, make each variant a leaf that references it (the coupling relocates into one explicit contract, not implicit cross-file consistency), so each leaf is narrow, delegatable, parallel, and the gate verifies it against the contract. Inline is narrowed to small surgical edits + the coordination itself. Substrate-carried, so it steers every connected session.
...
- **The design gate runs from the shared package, and runs alongside the antagonistic review** (STDIO-510). Two changes that make the STDIO-507 gate genuinely useful:
  - **One source, two consumers — the gate is the published `@verevoir/design-gate`.** The gate tooling was loaded by dynamic import from a _local corpus checkout_ only, so a remote (github) corpus silently fell back to the practices review (observed live: the design-pack gate never fired because the configured corpus is remote). It now imports the **published `@verevoir/design-gate` package** — the same zero-dependency verifier the design capabilities emit into a produced pack (for that pack's CI), versioned, and the one aigency-web's executor will import too. Not a corpus fetch, not a second copy: `import { verifyFiles, renderTokenView } from '@verevoir/design-gate'`. The gate fires regardless of how the corpus is sourced; resolution no longer depends on a local checkout. (A type shim covers the package's `.mjs`-only ship.)
  - **Gate AND review, staged — not gate _instead of_ review.** A gated capability previously ran the deterministic gate _in place of_ the antagonistic reasoning review. But the live GOV.UK run showed the review catches what the gate cannot — wrong brand values, an action colour mapped to the wrong hue, missing DTCG composite types, unverified contrast — none of which a structural gate sees. So `enact_capability` now runs **both**: the cheap deterministic gate first (short-circuiting its findings so the worker fixes _structure_ before any review spend), then the reasoning review once the structure is sound — converging only when **both** pass. The gate proves structure; the review proves fidelity and judgement. Degrades to gate-only when no reasoning tier is configured; the enactment header records `verified by: <gate> gate + reasoning review (looped)`.

## 0.66.0 — 2026-06-30

- **A capability runs its own declared gate** (STDIO-507). A capability addressed as a tool has to carry its own verification — a tool validates its output, it doesn't leave that to the caller. So when an enacted capability declares a deterministic gate (`descriptor.verify`, e.g. `design-pack`) and the MCP can run it, `enact_capability` now **enforces** that gate rather than only practices-reviewing the result: the gate is injected as the verifier into `delegate`'s existing produce→verify loop, so the worker is looped on the gate's findings until it passes. A new pure module (`src/tools/design-gate.ts`) gives the MCP access to the corpus's **own** zero-dependency design tooling — it dynamically loads `verifyFiles` + `renderTokenView` from a local corpus checkout, extracts the produced DTCG token JSON (fenced or prose-wrapped), builds the in-memory pack (token file + generated view), and runs `verifyFiles` over it, returning the verdict as a `VerifyResult` that plugs straight into the loop. It runs the **one** source in the corpus, not a second copy — the same gate aigency-web runs. A remote corpus (no local tooling) or an unknown gate name resolves to null and verification falls back to the reasoning-tier practices review (honest, non-blocking); the enactment header records which path ran (`verified by: design-pack gate (deterministic, looped)` vs the reasoning review). Validated end-to-end: a worker-tier produce of GOV.UK web tokens from the real source passed the `design-pack` gate clean for ~$0.01.
- **`enact_capability` — the structural executor** (STDIO-507). A new tool that makes the governed, verified, tiered handoff something the caller invokes in **one named call** rather than assembling by hand turn by turn. You name a **capability** (a corpus descriptor — what a good output looks like, its postcondition, its gate) and a **directive** (what to produce, in this project); the tool loads the descriptor, folds its intent into a self-contained task, provisions the bar and sends it down with the work (governed), produces on the **worker tier**, and — by default — verifies the result against the bar on the **reasoning tier**, looping the worker on the review's findings until it passes. It composes the pieces that already existed (`provision` for the bar, `delegate` for tiered+governed production with built-in antagonistic review, the capability corpus loader) behind one door, so the tiering and the verify are applied **structurally** instead of left to the session's per-turn judgement (which STDIO-508 showed a capable session simply skips — guidance is not enforcement). Unknown capability names return the available list rather than failing; an unreadable corpus degrades to a legible note. The injected doctrine (`instructions.md`) now names `enact_capability` as the single door for capability-shaped work, ahead of free-form `delegate` / `dispatch`. Pure-function core (`findDescriptor`, `buildEnactmentPrompt`, `enactmentHeader`) with `delegate` + the corpus loader injected for tests; audit-spanned (`enact:<type>`, capability kind) so it threads the flame cascade. (Stacks after 0.65.0 / PR #97.)
...
- **Flame by money, not just time — `verevoir-audit-trace --by time|cost`** (STDIO-506). The audit-trace flame chart could only be weighted by wall-clock time; a new `--by cost` re-weights it by **USD cost** — "where did the money go". A new pure module (`src/audit-cost.ts`) prices each span — using its exact `attributes.cost` when present (MCP delegate/dispatch model spans carry one), else computing `tokens_in×rateIn + tokens_out×rateOut + cached×rateCacheRead` from a small embedded family-substring rate table (opus/sonnet/haiku/deepseek/gemini/gpt + a default; approximate list prices, a _visualisation weight_ not a bill; cache-read ≈ 0.1× input), else 0. Because cost isn't a timeline, `relayoutByCost` rebuilds the cascade from `parent_span_id`, rolls each subtree's cost up, and re-lays-out every span so its width is its rolled-up cost with children packed left-to-right _inside_ the parent (contained, non-overlapping). The Chrome-trace view carries the cost-derived `ts`/`dur` in µ-dollars (`round(usd × 1e6)`, $0.01 → 10000, self-consistent with the time view's ms→µs) and surfaces each span's `cost_usd` in the detail panel; `--otlp --by cost` emits the same re-laid-out timestamps so the money weighting also travels through OTLP. Composes with `--from-claude-transcript` (so a Claude Code run — which carries tokens but no cost — flames by _computed_ money), `--otlp`, `-o`, and `--elide-notes`. Default stays `time` (unchanged behaviour); an unrecognised `--by` value is rejected loudly. Zero dependencies.
